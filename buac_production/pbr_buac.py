...
def run(temperature, pressure, buoh_flow_in, acac_flow_in):
    ...
    buol_flow_out = aspen.Tree.FindNode("\Data\Streams\OUTPUT\Output\MOLEFLOW\MIXED\BUTANOL").Value
    ...
    if show_info:
        print("\n--- buol out : %s kmol/h ---" % buol_flow_out)
    ...
    response.update_result(catalyst_weight, buol_flow_out, water_flow_out, acac_flow_out, buac_flow_out, vap_fraction, temperature_out)
    return response
...
class PbrStatus:
    ...
    def constraints_violations(self):
        ...
        if self.out_buac_flow < buac_production_goal * 0.999:
            constraints_count += 1
        # Temperature bounds
