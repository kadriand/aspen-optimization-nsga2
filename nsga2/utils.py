...
class NSGA2Utils(object):
    def __init__(self, problem, num_of_individuals, mutation_strength=0.2, num_of_genes_to_mutate=2, num_of_tour_particips=2):
    ...
    def crowding_operator(self, individual, other_individual):
        ...
        if individual.violated_constraints < other_individual.violated_constraints:
            return 1
    ...
    def __mutate(self, child):
        ...
        for gene in genes_to_mutate:
            span = pbr_props.bounds[1][gene] - pbr_props.bounds[0][gene]
            child.features[gene] = child.features[gene] - self.mutation_strength * span / 2 + random.random() * self.mutation_strength * span

            if child.features[gene] < pbr_props.bounds[0][gene]:
                child.features[gene] = pbr_props.bounds[0][gene]
            elif child.features[gene] > pbr_props.bounds[1][gene]:
                child.features[gene] = pbr_props.bounds[1][gene]
